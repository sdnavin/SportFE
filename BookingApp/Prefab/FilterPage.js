...
export default class FilterPage extends Component {
    ...
    render() {
        return (
            ...
            <Text style={styles.button}>Clear All</Text></TouchableOpacity>
            <TouchableOpacity onPress={()=>{this.props.navigation.goBack()}}>
            <Text style={styles.button}>Apply</Text></TouchableOpacity>
            </View>
            <View style={{width:'100%',height:50,flexDirection:'row',}}>
            {/* <View style={{flexDirection:'row',alignItems:'space-between'}}> */}
            <TouchableOpacity onPress={()=>this.setState({uiOption:0})} style={styles.tabbutton} ><Text style={{alignSelf:'center',color:'black',fontWeight:'bold'}}>Avaliable</Text></TouchableOpacity>
            </View>
            <View style={{width:'100%',height:80,flexDirection:'row',alignItems:'center'}}>
            <Text style={{flex:0.3,paddingLeft:'10%'}}>Date</Text>
            <TouchableOpacity style={{borderWidth:0.5,borderRadius:10,flex:0.5,height:'45%',marginEnd:10,justifyContent:'center'}} onPress={()=>this.setState({showDateTime:1})}>
            <Text style={{alignSelf:'center',fontSize:16,fontWeight:'500'}} >{this.state.selectdate.getDate() + "-" + (this.state.selectdate.getMonth() + 1) + "-" + this.state.selectdate.getFullYear()}</Text></TouchableOpacity><Fontisto name='date' size={18} color='black'/>
            </View>
        ...
        <Text style={{paddingLeft:10}}>{'Start Time'}</Text>
        {UIElements.drawGapH(15)}
        <TouchableOpacity style={{borderWidth:0.5,borderRadius:10,flex:0.5,height:'45%',marginEnd:10,justifyContent:'center'}} onPress={()=>this.setState({showDateTime:2})}>
        <Text style={{alignSelf:'center',fontSize:16,fontWeight:'500'}}>{this.state.showStartTime}</Text></TouchableOpacity>
        <Text>{'End Time'}</Text>
        {UIElements.drawGapH(10)}
        <TouchableOpacity style={{borderWidth:0.5,borderRadius:10,flex:0.5,height:'45%',marginEnd:10,justifyContent:'center'}} onPress={()=>this.setState({showDateTime:3})}>
        <Text style={{alignSelf:'center',fontSize:16,fontWeight:'500'}}>{this.state.showEndTime}</Text></TouchableOpacity>
        </View>
    ...
    <TouchableOpacity onPress={()=>this.setState({uiOption:1})}  style={styles.tabbutton}  ><Text style={{alignSelf:'center',color:'black',fontWeight:'bold'}}>Sports</Text></TouchableOpacity>
    </View>
                <FlatGrid 
                style={{margin:10}}
                itemDimension={100}
    ...
    const styles= StyleSheet.create({
        header:{
            backgroundColor:colors.appcolor,flexDirection:'row'
            // ,height:'15%'
        },title:{
            padding:16,
            // padding:15,
